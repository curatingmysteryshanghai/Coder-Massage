...
REPO_ROOT=$(CDPATH= cd -- "$ROOT_DIR/../.." 2>/dev/null && pwd || true)

# When this package still lives inside the complete Jieya repository, keep one
# installation path and one canonical marketplace identity.
if [ -n "$REPO_ROOT" ] &&
   [ "$ROOT_DIR" = "$REPO_ROOT/games/needlewhile" ] &&
   [ -f "$REPO_ROOT/install.sh" ] &&
   [ -f "$REPO_ROOT/.agents/plugins/marketplace.json" ]; then
...
CODEX_DOCTOR="$ROOT_DIR/scripts/codex-hook-doctor.mjs"
...
if [ "$NODE_MAJOR" -lt 18 ]; then
  echo "Needlewhile requires Node.js 18 or newer; found $(node --version 2>/dev/null || echo unknown)." >&2
  exit 69
fi
